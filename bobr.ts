...
const utils = {
  tableEntires: (bobrFile: any) => Object.entries<{
    partition_key: {name: string, type: keyof typeof attributeType},
    sort_key?: {name: string, type: keyof typeof attributeType},
  }>(bobrFile.dynamodb.table),
...
const cdkLocal = (bobrFile: any, rootDir: Directory) => {
  const localDir = rootDir.createDirectory('local');

  const dockerComposeFile = localDir.createSourceFile(
    'docker-compose.yaml',
...
`,
  ...
  let dynamoDBSetupFile: SourceFile | undefined;
  ...
  if(bobrFile.dynamodb) {
    ...
    tables.forEach(([tableName, schema]) => {
      dynamoDBSetupFile?.addStatements(`await dynamoDBClient.deleteTable({ TableName: '${tableName}' });`);

      dynamoDBSetupFile?.addStatements(`await dynamoDBClient.createTable({
        TableName: '${tableName}',
  ...
  return {dockerComposeFilePath: dockerComposeFile.getFilePath(), dynamoDBSetupFilePath: dynamoDBSetupFile?.getFilePath()};
}

const cdkStacks = (bobrFile: any, rootDir: Directory) => {
